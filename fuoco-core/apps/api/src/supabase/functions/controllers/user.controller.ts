...
import {Controller, Post, Guard, ContentType} from 'https://fuoco-appdev-core-api-tbm0aj6nffc0.deno.dev/core/src/index.ts';
...
import { GettingStartedRequest, User, UserRequestStatus } from '../protobuf/core_pb.js';
...
export class UserController {
    ...
    @Guard(AuthGuard)
    @ContentType('application/x-protobuf')
    public async requestGettingStartedAsync(context: Oak.RouterContext<string, Oak.RouteParams<string>, Record<string, any>>): Promise<void> {
        const token = context.request.headers.get("session-token") ?? '';
        const supabaseUser = await SupabaseService.client.auth.api.getUser(token);
        if (!supabaseUser.user) {
            throw HttpError.createError(404, `Supabase user not found`); 
        ...
        const body = await context.request.body();
        const requestValue = await body.value;
        const gettingStartedRequest = GettingStartedRequest.deserializeBinary(requestValue);
        ...
        const updatedUserData = await UserService.updateAsync(supabaseUser.user.id, partialUser);
        if (!updatedUserData) {
            throw HttpError.createError(404, `Supabase user not updated`); 
        }

        const responseUser = await UserService.assignAndGetUserProtocol(updatedUserData);
        context.response.type = "application/x-protobuf";
        context.response.body = responseUser.serializeBinary();
    }
