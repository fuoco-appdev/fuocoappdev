...
export class AccountNotificationController {
  private readonly _accountNotificationService: AccountNotificationService;

  constructor() {
    this._accountNotificationService = serviceCollection.get(
  ...
  public async handleWebhookOrderAsync(
    ...
    if (type === 'INSERT') {
      await this._accountNotificationService.createOrderNotificationAsync(
        order
      );
    }
  ...
  public async handleWebhookAccountFollowersAsync(
    ...
    const type = body['type'];
    if (type === 'UPDATE') {
      await this._accountNotificationService.createAccountFollowerNotificationAsync(
        accountFollower
      );
    }
  ...
  public async getNotificationsAsync(
    ...
    const response =
      await this._accountNotificationService.getNotificationsAsync(request);
    if (!response) {
  ...
  public async updateSeenAllAsync(
    ...
    const response = await this._accountNotificationService.updateSeenAllAsync(
      paramsAccountId
    );
    if (!response) {
  ...
  public async getUnseenCountAsync(
    ...
    const paramsAccountId = context.params['accountId'];
    const response = await this._accountNotificationService.getUnseenCountAsync(
      paramsAccountId
    );
    if (!response) {
      throw HttpError.createError(
