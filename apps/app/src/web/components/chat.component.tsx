...
import { ChatDocument, ChatSeenMessage, ChatState, DecryptedChatMessage, DecryptedChatMessages } from '../../models/chat.model';
...
export default function ChatComponent(): JSX.Element {
    ...
    const getLastSeenMessages = (messages: DecryptedChatMessage[]) => {
        let orderedSeenMessages = messages.map((value) => chatProps.seenBy[value.id ?? '']);
        // Clean undefined array
        orderedSeenMessages = orderedSeenMessages.filter((item, index) => {
            return orderedSeenMessages.indexOf(item) == index && item !== undefined;
        });
        // 

        console.log(orderedSeenMessages);
    }
    ...
    useEffect(() => {
        ...
        getLastSeenMessages(decryptedMessages);
    }, [chatProps.messages, chatProps.seenBy, id]);
    ...
    useEffect(() => {
        ...
        const chatMessages: DecryptedChatMessages | undefined = Object.keys(chatProps.messages).includes(id) ? chatProps.messages[id] : undefined;
        ...
        const updateSeenMessage = async () => {
            ...
            const firstMessage = chatMessages.messages.at(chatMessages.messages.length - 1) as DecryptedChatMessage;
