...
class MapboxService extends Service {
  ...
  constructor() {
    ...
    this._accessToken = process.env["MAPBOX_ACCESS_TOKEN"] ?? "";
  }
  ...
  public async requestGeocodingPlacesAsync(
    ...
    const geocodingResponse = await axios.get(
        `${this._geocodingUrl}/mapbox.places/${searchText}.json?&access_token=${this._accessToken}&language=${language}&types=${types.join(',')}`
      );
  ...
  public async requestReverseGeocodingPlacesAsync(
    ...
    const geocodingResponse = await axios.get(
        `${this._geocodingUrl}/mapbox.places/${geo.lng},${geo.lat}.json?&access_token=${this._accessToken}&language=${language}&types=${types.join(',')}`
      );
      return geocodingResponse.data;
  }
}
